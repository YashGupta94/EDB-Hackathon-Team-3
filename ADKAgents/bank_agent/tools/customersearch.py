...
def customer_id_search(customer_id: str, tool_context: ToolContext) -> dict:
    ...
    """
    if not BQ_DATASET:
        return {"status": "error", "error_message": "BQ_DATASET is not configured. Check bank_agent/.env."}
    ...
    try:
        ...
        print(f"[customersearch] customer_id_search ERROR: BQ_DATASET={BQ_DATASET!r} PROJECT={PROJECT_ID!r} err={type(e).__name__}: {e}")
...
def customer_database_search(tool_context: ToolContext) -> str:
    ...
    """
    if not BQ_DATASET:
        return "ERROR: BQ_DATASET is not configured. Check bank_agent/.env."
    ...
    try:
        ...
        print(f"[customersearch] customer_database_search ERROR: {type(e).__name__}: {e}")
